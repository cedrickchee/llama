...
# 8-bit quantized LLaMA model.
#
# All Model Parallel (MP) stuffs removed.
# So you can run model in a single GPU.
# 
...
class Int8Linear(torch.nn.Module):
    def __init__(self, float_linear: nn.Linear) -> None:
        super().__init__()
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs):
        ...
        self.output = UninitializedLinear(
    ...
    def quantize(self):
        # change all layers
        #
        # `get_layer` and `set_layer` have the same signature as `getattr` and
        # `setattr` but will handle the sub-modules.
